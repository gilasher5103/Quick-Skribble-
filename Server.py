...
def end_game(wlist):
    ...
    max_points = 0
    winner = ""
    for tup in scoreboard:
        if tup[1] > max_points:
            max_points = tup[1]
            winner = tup[0]
        elif tup[1] == max_points:
            winner = winner + " and " + tup[0]
    if max_points == 0:
        winner = "NO ONE"
    print("THE FINAL SCOREBOARD IS: " )
    print(scoreboard)
...
def handle_queue(wlist): #alerts the next player that it is now his turn
    ...
    print(queue_list)
    random_hidden_word()
    message = "Your turn" +hidden_word +" END OF 'YOUR TURN' Message "
    print("HIDDEN WORD MESSAGE IS: " + message)
    if queue_list == []:
        if round_number < 3:
